/*********************************************************************************
*                              Author: Alexy Heitz                               *
*                        File Name: /CPP-01/ex06/Harl.cpp                        *
*                    Creation Date: January 21, 2025 08:20 AM                    *
*                    Last Updated: January 21, 2025 10:38 AM                     *
*                              Source Language: cpp                              *
*                                                                                *
*                            --- Code Description ---                            *
...
void	Harl::debug(void) {
	...
	std::cout << BG_GREEN << "[ DEBUG ]" << RESET << " - Harl: " << GREEN << someComplaints[rand() % 5] << RESET << std::endl;
}
...
void	Harl::info(void) {
	...
	std::cout << BG_BLUE << "[ INFO  ]" << RESET << " - Harl: " << BLUE << someComplaints[rand() % 5] << RESET << std::endl;
}
...
void	Harl::warning(void) {
	...
	std::cout << BG_YELLOW << "[WARNING]" << RESET << " - Harl: " << YELLOW << someComplaints[rand() % 5] << RESET << std::endl;
}
...
void	Harl::error(void) {
	...
	std::cout << BG_RED << "[ ERROR ]" << RESET << " - Harl: " << RED << someComplaints[rand() % 5] << RESET << std::endl;
}
...
void	Harl::complain(std::string level) {
	...
	for (index i = 0 ; i < 4 ; ++i) {
		if (equivalentString(level, availableLevels[i])) {
			for (index j = 0 ; j <= i ; j++)
				(this->*methods[j])();
			sleep(INTERACTION), erasePreviousLines(i + 1);
			return;
		}
	}
